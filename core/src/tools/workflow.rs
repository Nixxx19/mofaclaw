...
/// Initiate a workflow from within an agent event loop.
///
/// Similar to `start_workflow` but designed for agent self-initiation.
/// The `team_id` is optional; if omitted the first team found is used.
pub struct InitiateWorkflowTool {
    workflow_engine: Arc<WorkflowEngine>,
    team_manager: Arc<TeamManager>,
}

impl InitiateWorkflowTool {
    pub fn new(workflow_engine: Arc<WorkflowEngine>, team_manager: Arc<TeamManager>) -> Self {
        Self {
            workflow_engine,
            team_manager,
...
#[async_trait]
impl SimpleTool for InitiateWorkflowTool {
    fn name(&self) -> &str {
        "initiate_workflow"
    }

    fn description(&self) -> &str {
        "Autonomously initiate a multi-agent workflow (for use by agents inside event loops). \
         Returns a workflow_id you can pass to get_workflow_status. \
         Available workflows: 'code_review', 'design'"
    }
    ...
    fn parameters_schema(&self) -> serde_json::Value {
        json!({
            ...
            "properties": {
                "workflow_name": {
                    "type": "string",
                    "description": "Workflow to run: 'code_review' or 'design'",
                    "enum": ["code_review", "design"]
                },
                "team_id": {
                    "type": "string",
                    "description": "Team ID to run the workflow with (optional, uses first available team if omitted)"
                },
                "context": {
                    "type": "object",
                    "description": "Initial context variables, e.g. {\"user_request\": \"...\"}",
                    "additionalProperties": true
                }
            },
    ...
    async fn execute(&self, input: ToolInput) -> ToolResult {
        ...
        let initial_context: HashMap<String, serde_json::Value> =
            match input.get::<serde_json::Value>("context") {
                Some(v) => {
                    if let Some(obj) = v.as_object() {
                        obj.iter().map(|(k, v)| (k.clone(), v.clone())).collect()
                    } else {
        ...
        // Resolve team — use provided team_id or the first available team
        let team = if let Some(team_id) = input.get_str("team_id") {
            ...
            match teams.first() {
                Some(team_id) => match self.team_manager.get_team(team_id).await {
                    Some(t) => t,
                    None => return ToolResult::failure("Could not load team"),
                },
                None => return ToolResult::failure("No teams available to run the workflow"),
            }
        };

        let workflow = match workflow_name {
            "code_review" => crate::agent::collaboration::create_code_review_workflow(),
            "design" => crate::agent::collaboration::create_design_workflow(),
            _ => return ToolResult::failure(format!("Unknown workflow: '{}'", workflow_name)),
        };
        ...
        ToolResult::success_text(
            serde_json::to_string(&json!({
                "workflow_id": workflow_id,
                "status": "initiated",
                "message": "Workflow initiated by agent. Poll get_workflow_status for progress."
            }))
            .unwrap_or_else(|_| format!("Workflow initiated: {}", workflow_id)),
        )
    }
